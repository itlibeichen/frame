...
$(function(){
    ...
    (function(){
        ...
        $(window).resize(function() {//避免resize频繁的调用
    ...
    (function(){
        ...
        //插入元素
        $("#notices").append($("#notices a:first").clone());
        //获取item的高度
        var h = $("#notices a").outerHeight();
        var t = 3000;
        var t2 = 1000;
        var count = $("#notices a").length;
        var $notice = $("#notices");
        var j=0;
        var interval=null;
        //console.log(h);
        function ani(){
            ...
            interval = setInterval(function(){
               // console.log("动画！！");
                ...
                if(j>=count-1){
                    clearInterval(interval);
                    $notice.animate({top:-h*j},t2,function(){
                        $notice.css({top:"0px"});
                        ani();
                        j=0;
                    });
                }else {
                    $notice.animate({top: -h * j}, t2);
                }
            },t);
        }
        ani();
    ...
    //ucenter.js
    //账户左边
    $('.account-tit li').hover(function(){
        $(this).children(".color").css("display","block");
        $(this).children(".cl").hide();
        var className=$(this).children("i").attr("class");
        var classLength=className.length;//长度
        var icon = $(this).children("i").attr("class");
        var icon_new = icon + '-1';
        $(this).children('i').removeClass().addClass(icon_new);
        ...
        var className=$(this).children("i").attr("class");
        var classLength=className.length;//长度
        var icon_new = className.substring(0,classLength-2);
        $(this).children('i').removeClass().addClass(icon_new);
    ...
    $(".checkbox").click(function(){
        ...
        if($(this).hasClass("icon-8")){//未选中
            $(this).attr("class","icon-9");
            $("#"+target).attr("checked", true);
            return false;
        }
        $(this).attr("class","icon-8");
        $("#"+target).attr("checked", false);
        return false;
    });
    //帮助中心
    $('.help-center-con li').hover(function() {
        $(this).children(".con").show();
        $(this).children("span").children("a").css("color","#359df6");
        $(this).children("i").removeClass("icon-33").addClass("icon-34");
    }, function(){
        $(this).children("span").children("a").css("color","#333");
        $(this).children(".con").hide();
        $(this).children("i").removeClass("icon-34").addClass("icon-33");
    });
    //账户设置
    $(".open").click(function(){
        $(this).parent().siblings().children(".name-weight").css("font-weight","bold");
        $(this).parent("div").siblings(".hide-con").show();
        $(this).hide().siblings(".up").show();
        return false;
    });
    $(".up").click(function(){
        $(this).parent().siblings().children(".name-weight").css("font-weight","normal");
        $(this).parent("div").siblings(".hide-con").hide();
        $(this).hide().siblings(".open").show();
        return false;
    });
    $('.click-tit li').click(function(){
        $(this).addClass('active').siblings().removeClass('active');
        var str = $(this).attr('class');
        var num = str.substr(7,1);
        var id_str = '#tab' + num;
        $(this).parents(".click-tit").siblings("div[id^='tab']").hide();
        $(id_str).show();
    });
    ...
    $.prototype.MoveUp = function MoveUp(arg1,arg2){
        ...
        if (inner.scrollHeight >= outer.offsetHeight ) {
            var moveUp = function () {
                var innerTop = parseFloat(inner.style.top);

                inner.style.height = -innerTop + "px";
                innerTop = innerTop - 1;//每100毫秒向上滚动1px，滚动速度。---可修改
                inner.style.top = innerTop + "px";
                if (innerTop + inner.scrollHeight <= outer.offsetHeight) {//这个数值是根据outer的高度来设置的，判断当inner的内容已经向上滚到了outer的底部
                    //在把之前的内容向末尾增加一遍,这样就可以继续向后走了,从而实现我们的无缝衔接自动滚动
                    inner.innerHTML += val;
                }
            };
...
$(function(){
	var o = {
		...
		paper: function(){
			var r = Raphael(o.paperDiv, 41, 41),
				rad = 19,//���ߵ�λ�ö�λ
				defaultText = o.cicleNum+'%';
				var proceInt=parseInt(defaultText);
				speed = 250;
			
			var rc=r.circle(20, 20,19);//15 ����λ�ö�λ
				rc.attr({ stroke: "#e6e6e6", "stroke-width": 2});//���߿���
			var title = r.text(20, 20, defaultText).attr({
